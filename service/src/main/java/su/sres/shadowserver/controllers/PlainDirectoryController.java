  ...
  public PlainDirectoryController(RateLimiters rateLimiters, AccountsManager accountsManager) {
    ...
    private DirectoryResponse fullDirectoryResponse(long version) {
		...
		return DirectoryResponse.newBuilder()
			...
			// if directory restore is currently in progress, we simply return no-update in order to avoid a possible race condition 
	...
	private DirectoryUpdate getFullDirectory() {
		...
		if (!retrievedPlainDirectory.isEmpty()) {
			...
			return DirectoryUpdate.newBuilder()
			    ...
			    // plain directory should never be empty; if it's not the case then something is wrong with Redis and we need to recreate it from SQL
			    
			    // getFullDirectory() should not be invoked while the directory restoration lock is set, so there should be no race condition here				
