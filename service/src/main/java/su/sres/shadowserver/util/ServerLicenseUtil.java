...
import su.sres.shadowserver.WhisperServerService;
...
public class ServerLicenseUtil {
	...
	try (LicenseReader reader = new LicenseReader(filepath + "/" + Constants.serverLicenseFilename)) {
	    ...
	    } else return new Pair<LicenseStatus, Pair<Integer, Integer>>(LicenseStatus.ABSENT, new Pair<Integer, Integer>(-1, actualUsers));
	}
    ...
    private static boolean featuresOK(License license) {
