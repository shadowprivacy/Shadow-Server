...
import su.sres.shadowserver.storage.mappers.OutgoingMessageEntityRowMapper;
...
public class Messages {
	...
	database.use(jdbi -> jdbi.useTransaction(handle -> {
		...
		final PreparedBatch batch = handle
			.prepareBatch("INSERT INTO messages (" + GUID + ", " + TYPE + ", " + RELAY + ", " + TIMESTAMP + ", " + SERVER_TIMESTAMP + ", " + SOURCE + ", " + SOURCE_UUID + ", " + SOURCE_DEVICE + ", " + DESTINATION + ", " + DESTINATION_DEVICE + ", " + MESSAGE + ", " + CONTENT + ") " +
	...
	return database.with(jdbi -> jdbi.withHandle(handle -> {
		...
		return handle
			.createQuery("DELETE FROM messages WHERE " + ID + " IN (SELECT " + ID + " FROM messages WHERE " + DESTINATION + " = :destination AND " + DESTINATION_DEVICE + " = :destination_device AND " + SOURCE + " = :source AND " + TIMESTAMP + " = :timestamp ORDER BY " + ID
				+ " LIMIT 1) RETURNING *")
			.bind("destination", destination)
			.bind("destination_device", destinationDevice)
			.bind("source", source)
