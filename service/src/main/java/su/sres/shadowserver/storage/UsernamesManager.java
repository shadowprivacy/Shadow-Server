...
public class UsernamesManager {
	...
	private static final Timer redisUsernameGetTimer = metricRegistry
			.timer(name(AccountsManager.class, "redisUsernameGet"));
	...
	private Optional<UUID> redisGet(String username) {
		try (Jedis jedis = cacheClient.getReadResource(); Timer.Context ignored = redisUsernameGetTimer.time()) {
	...
	private Optional<String> redisGet(UUID uuid) {
		try (Jedis jedis = cacheClient.getReadResource(); Timer.Context ignored = redisUuidGetTimer.time()) {
	...
	private void redisDelete(UUID uuid) {
		try (Jedis jedis = cacheClient.getWriteResource(); Timer.Context ignored = redisUuidGetTimer.time()) {
			Optional<String> username = redisGet(uuid);
