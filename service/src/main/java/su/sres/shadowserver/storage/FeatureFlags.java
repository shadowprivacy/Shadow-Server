...
/**
 * The feature flag database is a persistent store of the state of all
 * server-side feature flags. Feature flags are identified by a human-readable
 * name (e.g. "invert-nano-flappers") and are either active or inactive.
 * <p/>
 * The feature flag database provides the most up-to-date possible view of
 * feature flags, but does so at the cost of interacting with a remote data
 * store. In nearly all cases, callers should prefer a cached,
 * eventually-consistent view of feature flags (see
 * {@link FeatureFlagsManager}).
 * <p/>
 * When an operation requiring a feature flag has finished, callers should
 * delete the feature flag to prevent accumulation of non-functional flags.
 */
public class FeatureFlags {
    ...
    private static final RowMapper<Pair<String, Boolean>> PAIR_ROW_MAPPER = (resultSet, statementContext) -> new Pair<>(resultSet.getString("flag"), resultSet.getBoolean("active"));
