...
/**
 * An abstract base class that assembles a real (local!) Redis cluster and
 * provides a client to that cluster for subclasses.
 */
public abstract class AbstractRedisClusterTest {
	...
	try (final StatefulRedisConnection<String, String> connection = waitClient.connect()) {
	    // CLUSTER INFO gives us a big blob of key-value pairs, but the one we're
	    // interested in is `cluster_state`.
	    // According to https://redis.io/commands/cluster-info, `cluster_state:ok` means
	    // that the node is ready to
	    // receive queries, all slots are assigned, and a majority of master nodes are
	    // reachable.
