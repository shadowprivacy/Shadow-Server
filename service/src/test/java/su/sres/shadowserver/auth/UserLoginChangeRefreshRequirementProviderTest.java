...
class UserLoginChangeRefreshRequirementProviderTest {

  private UserLoginChangeRefreshRequirementProvider provider;
  ...
  @BeforeEach
  void setUp() {
    provider = new UserLoginChangeRefreshRequirementProvider();
  ...
  void handleRequestNumberChange() {
    ...
    provider.handleRequestFiltered(request);
    when(account.getUserLogin()).thenReturn(CHANGED_NUMBER);
