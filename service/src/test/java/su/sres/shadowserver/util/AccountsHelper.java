    ...
    if (mockingDetails.isMock()) {
      ...
      for (Stubbing stubbing : mockingDetails.getStubbings()) {
        switch (stubbing.getInvocation().getMethod().getName()) {
          ...
          case "getUserLogin": {
            when(updatedAccount.getUserLogin()).thenAnswer(stubbing);
            break;
          }
          case "getDevices": {
          ...
          case "isDiscoverableByUserLogin": {
            when(updatedAccount.isDiscoverableByUserLogin()).thenAnswer(stubbing);
            break;
          }
          ...
          case "isChangeUserLoginSupported": {
            when(updatedAccount.isChangeUserLoginSupported()).thenAnswer(stubbing);
            break;
          }
      ...
      updatedAccount.setUserLogin(account.getUserLogin());
      account.markStale();
    }
