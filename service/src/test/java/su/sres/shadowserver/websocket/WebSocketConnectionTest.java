...
public class WebSocketConnectionTest {
	...
	when(storedMessages.getMessagesForDevice(account.getUserLogin(), account.getUuid(), device.getId(), userAgent, false))
	...
	verify(storedMessages, times(1)).delete(eq(account.getUserLogin()), eq(accountUuid), eq(2L), eq(outgoingMessages.get(1).getGuid()));
	...
	// This is a little hacky and non-obvious, but because the first call to
	// getMessagesForDevice returns empty list of
	// messages, the call to CompletableFuture.allOf(...) in processStoredMessages
	// will produce an instantly-succeeded
	// future, and the whenComplete method will get called immediately on THIS
	// thread, so we don't need to synchronize
	...
	when(messagesManager.getMessagesForDevice(account.getUserLogin(), account.getUuid(), 1L, client.getUserAgent(), false)).thenAnswer((Answer<OutgoingMessageEntityList>) invocation -> {
	...
	final List<OutgoingMessageEntity> firstPageMessages = List.of(createMessage(1L, false, "sender1", UUID.randomUUID(), 1111, false, "first"),
		createMessage(2L, false, "sender1", UUID.randomUUID(), 2222, false, "second"));

	final List<OutgoingMessageEntity> secondPageMessages = List.of(createMessage(3L, false, "sender1", UUID.randomUUID(), 3333, false, "third"));
	...
	when(messagesManager.getMessagesForDevice(account.getUserLogin(), account.getUuid(), 1L, client.getUserAgent(), false))
	...
	when(messagesManager.getMessagesForDevice(account.getUserLogin(), account.getUuid(), 1L, client.getUserAgent(), false)).thenReturn(firstPage);
	...
	// This is a little hacky and non-obvious, but because we're always returning an
	// empty list of messages, the call to
	// CompletableFuture.allOf(...) in processStoredMessages will produce an
	// instantly-succeeded future, and the
	// whenComplete method will get called immediately on THIS thread, so we don't
	// need to synchronize or wait for
	// anything.
	connection.processStoredMessages();
	connection.processStoredMessages();
	...
	verify(messagesManager).getMessagesForDevice(account.getUserLogin(), account.getUuid(), device.getId(), client.getUserAgent(), false);

	connection.handleNewMessagesAvailable();

	verify(messagesManager).getMessagesForDevice(account.getUserLogin(), account.getUuid(), device.getId(), client.getUserAgent(), true);
    }
	...
	verify(messagesManager, times(2)).getMessagesForDevice(account.getUserLogin(), account.getUuid(), device.getId(), client.getUserAgent(), false);
    }
	...
	String userAgent = "Shadow-Desktop/1.2.3";
	...
	// We should delete all three messages even though we only sent two; one got
	// discarded because it was too big for
	// desktop clients.
	verify(storedMessages, times(3)).delete(eq(account.getUserLogin()), eq(accountUuid), eq(2L), any(UUID.class));
