...
import su.sres.shadowserver.controllers.RemoteConfigController;
...
public class RemoteConfigControllerTest {
	...
	public final ResourceTestRule resources = ResourceTestRule.builder().addProvider(AuthHelper.getAuthFilter())
			...
			.addResource(new RemoteConfigController(remoteConfigsManager, remoteConfigsAuth)).build();

	@Before
	public void setup() {
		when(remoteConfigsManager.getAll()).thenReturn(new LinkedList<>() {
			{
				add(new RemoteConfig("android.stickers", 25, Set.of(AuthHelper.DISABLED_UUID, AuthHelper.INVALID_UUID),
						null, null));
	...
	public void testRetrieveConfigUnauthorized() {
		Response response = resources.getJerseyTest().target("/v1/config/").request()
	...
	public void testSetConfig() {
		Response response = resources.getJerseyTest().target("/v1/config").request().header("Config-Token", "foo")
				.put(Entity.entity(new RemoteConfig("android.stickers", 88, Set.of(), "FALSE", "TRUE"),
	...
	public void testSetConfigValued() {
		Response response = resources.getJerseyTest().target("/v1/config").request().header("Config-Token", "foo")
				.put(Entity.entity(new RemoteConfig("value.sometimes", 50, Set.of(), "a", "b"),
						MediaType.APPLICATION_JSON_TYPE));
	...
	public void testSetConfigUnauthorized() {
		Response response = resources.getJerseyTest().target("/v1/config").request().header("Config-Token", "baz")
				.put(Entity.entity(new RemoteConfig("android.stickers", 88, Set.of(), "FALSE", "TRUE"),
						MediaType.APPLICATION_JSON_TYPE));
	...
	public void testSetConfigMissingUnauthorized() {
		Response response = resources.getJerseyTest().target("/v1/config").request().put(Entity.entity(
				new RemoteConfig("android.stickers", 88, Set.of(), "FALSE", "TRUE"), MediaType.APPLICATION_JSON_TYPE));
	...
	public void testSetConfigEmptyName() {
		Response response = resources.getJerseyTest().target("/v1/config").request().header("Config-Token", "foo").put(
				Entity.entity(new RemoteConfig("", 88, Set.of(), "FALSE", "TRUE"), MediaType.APPLICATION_JSON_TYPE));
	...
	public void testDelete() {
		Response response = resources.getJerseyTest().target("/v1/config/android.stickers").request()
				.header("Config-Token", "foo").delete();
	...
	public void testDeleteUnauthorized() {
		Response response = resources.getJerseyTest().target("/v1/config/android.stickers").request()
				.header("Config-Token", "baz").delete();
	...
	public void testMath() throws NoSuchAlgorithmException {
		...
		SecureRandom secureRandom = new SecureRandom(new byte[] { 42 }); // the seed value doesn't matter so much as
																			// it's constant to make the test not flaky

		for (int i = 0; i < iterations; i++) {
			for (RemoteConfig config : remoteConfigList) {
				int count = enabledMap.getOrDefault(config.getName(), 0);

				if (RemoteConfigController.isInBucket(digest, getRandomUUID(secureRandom), config.getName().getBytes(),
						config.getPercentage(), new HashSet<>())) {
					count++;
				}
