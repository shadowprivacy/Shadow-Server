...
import su.sres.shadowserver.controllers.CertificateController;
...
public class CertificateControllerTest {
	...
	static {
		try {
			certificateGenerator = new CertificateGenerator(Base64.decode(signingCertificate),
					Curve.decodePrivatePoint(Base64.decode(signingKey)), 1);
		} catch (IOException e) {
	...
	public static final ResourceTestRule resources = ResourceTestRule.builder().addProvider(AuthHelper.getAuthFilter())
			...
			.addResource(new CertificateController(certificateGenerator)).build();
